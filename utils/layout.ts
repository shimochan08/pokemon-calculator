...
export function rebuildLayout(
    ...
    if (rows.length === 0) {
        rows.push([
            createRowAddPanel("none", items.length)
        ]);
    } else {
...
function fillRowWithAddPanels(
    ...
    const insertIndex =
        row.length > 0
            ? row[row.length - 1].originalIndex + 1
            : 0;
...
function mergeAddPanels(
    ...
    for (let i = 0; i < row.length; i++) {
        ...
        while (
            i + 1 < row.length &&
            row[i + 1].componentKey === "add_panel"
        ) {
        ...
        merged.push(
            createRowAddPanel(newSize, baseIndex)
        );
    }
