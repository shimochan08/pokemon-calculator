...
export default function PickedMember({ slotId, name, dex, ability, item, moves, nature, ivs, evs }: PickedMemberProps) {
  ...
  const pokemonAbilityText = abilityMap.find((t) => t.english === ability)?.japanese;
  const pokemonItemText = itemMap.find((t) => t.english === item)?.japanese;
  const pokemonNatureText = natureMap.find((t) => t.english === nature)?.japanese;
    ...
    <div className="member-card" onClick={handleClick}>
      ...
      {pokemon?.sprites.frontDefault && (
        <div className="member-pokemon">
          <img src={pokemon.sprites.frontDefault} alt={pokemon.name} className="member-sprite" />
          <div className="member-pokemon-name">{pokemonNameText ?? '不明なポケモン'}</div>
          <div className="member-pokemon-types">{pokemonTypesText && <span>{pokemonTypesText.join(' / ')}</span>}</div>
        </div>
      )}
    </div>
  );
    ...
    <div className="member-info">
      ...
      <div className="member-moves">
        {[0, 1, 2, 3].map((i) => {
          ...
          return (
            <div key={i} className={`move-box ${!move ? 'empty' : ''}`}>
              {move ?? ''}
            </div>
          );
        })}
