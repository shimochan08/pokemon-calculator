          ...
          <SearchBar
            ...
            onSelect={(p) => {
              ...
              setPokemonBuild((prev) =>
                prev
                  ? { ...prev, name: p.english, moves: [], ivs: defaultIvs, evs: defaultEvs, ability: '', item: '' }
                  : prev,
              );
            }}
          />
      ...
      {pokemon && (
        <div className="panel-main">
          <div className="panel-left">
            ...
            <div className="pokemon-name">{pokemonMap.find((t) => t.english === pokemon.name)?.japanese ?? '-'}</div>

            <div className="pokemon-type">
              <span>タイプ：</span>

              {pokemon.types.length === 1 ? <span>{pokemon.types[0]}</span> : <div>{pokemon.types.join(' / ')}</div>}
            </div>
          </div>
